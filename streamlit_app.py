...
@st.cache_data
def downloading_full_history(stock_symbol):
    all_data = []
...
# sidebar builder
with st.sidebar:
    st.image('images/MOEXlogo.png')
    st.title("Info")
    st.markdown("""
                The app predicts stock prices of [MOEX securities](https://www.moex.com/en/moexboard/instruments-list.aspx) 
                for the next 30 days from the current date. 
                The algorithm is based on an LSTM (Long Short-Term Memory) 
                [Recurrent Neural Network](https://en.wikipedia.org/wiki/Recurrent_neural_network), 
                and trained on the daily closing prices of the last 17 years.
                """)

    st.divider()
    st.subheader("*Disclaimer*")
    st.markdown("""*This stock price prediction model is for educational and informational purposes only. 
            The predictions generated by the model are not guaranteed and 
            should not be considered financial advice. 
            Stock markets are inherently volatile and influenced by many unpredictable factors. 
...
downloading_full_history(stock_symbol)
...
downloading_full_history.clear()